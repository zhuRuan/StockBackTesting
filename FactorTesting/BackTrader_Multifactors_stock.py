...
class momentum_factor_strategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        #已清洗过的每日可用股票列表
        self.df_byday = pd.read_csv(r'D:\Ruiwen\PythonProject\backtrader-master\data_stocks\2_stock_valid.csv')
        #交易天数，用于判断是否交易
        self.bar_num = 0
        #上次交易股票的列表
        self.last = []
...
#获取已清洗好的全A股列表
stocklist_allA = pd.read_csv(r'D:\Ruiwen\PythonProject\backtrader-master\data_stocks\2_stock_list.csv')
stocklist_allA.fillna(0)
stocklist_allA = stocklist_allA['0'].tolist()

#获取已清洗好的全A股所有数据
df_all = pd.read_csv(r'D:\Ruiwen\PythonProject\backtrader-master\data_stocks\2_stock_data.csv')
...
#考虑印花税和佣金印花税为0
comminfo=stampDutyCommissionScheme(stamp_duty=0.003,commission=0.0005)
cerebro.broker.addcommissioninfo(comminfo)

for s in stocklist_allA:
    feed = Addmoredata(dataname = get_stock_data(s),plot=False,
                       fromdate=datetime.datetime(2022,4,1),todate=datetime.datetime(2022,6,30))
